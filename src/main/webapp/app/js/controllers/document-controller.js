angular.module("accounting").controller('DocumentController',function($scope,$rootScope,$routeParams, DocumentService) {
	...
	$scope.initHomePage = function() {
		...
		//$scope.findAllDocumentsByTitleAndServiceSelected();
		$scope.findMyAccountDetail();
		//$scope.findDocumentByServiceSelected();
	}
	
	$scope.findDocumentByServiceSelected = function(){
		$scope.loggedInUser = JSON.parse(localStorage.getItem('loggedInUser'));
		if(typeof($routeParams.type) !='undefined'){
			var qry = "?userId="+$scope.loggedInUser.userId+"&serviceSelected="+$routeParams.type;
			
			DocumentService.findAllDocumentsByServiceSelected(qry).then(function(response){
				hideLoader();
				$scope.document = [];
				
				var documentsMap = response.data;
				if (documentsMap['document']) {
					$scope.contents = documentsMap['document'];
				}
	...
	$scope.findAllDocumentsByTitleAndServiceSelected = function(title) {
		showLoader();
		window.localStorage.removeItem("selectedDocument");
		$scope.loggedInUser = JSON.parse(localStorage.getItem('loggedInUser'));
		var qry = "?userId="+$scope.loggedInUser.userId;
		...
		if ($routeParams.type) {
			qry = qry+"&serviceSelected="+$routeParams.type;
		}
		DocumentService.findAllDocumentsByTitleAndServiceSelected(qry).then(function(response){
			hideLoader();
			$scope.document = [];
	...
	$scope.loadDocumentsByTitle =function(searchTitle){
		...
		$scope.findAllDocumentsByTitleAndServiceSelected(searchTitle);
	}
