...
angular.module("accounting").controller('UserController',function($scope, $rootScope,UserService, Facebook) {
	...
	$scope.updateSpecialAccount = function(userId,specialAccount){
		$scope.user = {};
		$scope.user.userId = userId;
		$scope.user.specialAccount = specialAccount;
		UserService.updateUser($scope.user).then(function(response) {
			...
			if (response.data.errorCode != 0) {
				...
				toastr["success"]("User Special Account status has been updated");
			}
		});
	}
});