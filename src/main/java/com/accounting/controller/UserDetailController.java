...
public class UserDetailController {
	...
	@Autowired
    com.accounting.service.UserService userDetailService;
	...
	@RequestMapping(value = "/fetch/all/users", method = RequestMethod.GET)
	@ResponseBody
	public ResponseEntity<List<Map<String,String>>> fetchAllUsers() {
		try {
			
			List<Map<String,String>> userList = accountingReportDao.getAllUsers();
			
			return new ResponseEntity<List<Map<String,String>>>(userList,
					HttpStatus.OK);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	@RequestMapping(value = "api/update/specialAccount", method = RequestMethod.POST)
	@ResponseBody
	public ResponseEntity<User> updateUserSpecialAccountStatus(HttpServletResponse httpServletResponse, @RequestBody User user) {
	
			User userInfo = null;
			userInfo = userDetailService.findUserByUserId(user.getUserId());
			if (userInfo != null) {
				try {
					userInfo.setSpecialAccount(user.isSpecialAccount());
					userInfo = userDetailService.saveUser(userInfo);
				} catch (Exception e) {
					e.printStackTrace();
					return new ResponseEntity<User>(user, HttpStatus.OK);
				}
