...
public class SearchController {
	...
	public Map<String,List<UserDocument>> findTopTenDocuments(Long userId,String title) {
		...
		if (myAccount != null && myAccount.getMainCourseId() != null && myAccount.getSecondryCourseId() != null) {
			...
			boolean bySelectedService = false;
			List<String> selectedService = new ArrayList<String>();
			if(myAccount.isAccomodationTaken()) {
				selectedService.add("accomodation");
				bySelectedService = true;
			}
			if(myAccount.isCoachingAdsOn()) {
				selectedService.add("coachingAds");
				bySelectedService = true;
			}
			if(myAccount.isJobsOn()) {
				selectedService.add("jobs");
				bySelectedService = true;
			}
			if(myAccount.isMealTaken()) {
				selectedService.add("meals");
				bySelectedService = true;
			}
			if(myAccount.isStatutesOn()) {
				selectedService.add("statutes");
				bySelectedService = true;
			}
			if(!bySelectedService) {
				...
				topTenDocMap.put("content", documentDao.findTop10ContentDocumentsByService(selectedService,title));
			}
			
		} else {
			topTenDocMap.put("image", new ArrayList<UserDocument>());
			topTenDocMap.put("video",  new ArrayList<UserDocument>());
			topTenDocMap.put("content", new ArrayList<UserDocument>());
		}
		return topTenDocMap;
	}
	
	@RequestMapping(value="/find/documents/ByTitleAndServiceSelected",produces="application/json")
	public Map<String,List<UserDocument>> findDocumentsByTitleAndServiceSelected(Long userId,String title,String serviceSelected) {
		Map<String,List<UserDocument>> docMap = new HashMap<>();
	    docMap.put("document", documentDao.findDocumentsByServiceSelectedAndTitle(serviceSelected,title));
	return docMap;
	}
	
	@RequestMapping(value="/find/documentsByServiceSelected",produces="application/json")
	public Map<String,List<UserDocument>> findDocumentsByServiceSelected(String serviceSelected) {
		Map<String,List<UserDocument>> docMap = new HashMap<>();
		docMap.put("document", documentDao.findDocumentsByServiceSelected(serviceSelected));
		return docMap;
	}
