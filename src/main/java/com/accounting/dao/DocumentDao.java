...
public class DocumentDao {
	...
	public List<UserDocument> findTop10ContentDocumentsByService(List<String> services, String title){
StringBuffer sb = new StringBuffer();
		String serviceSelected ="";
		for (String srvSel : services){
			serviceSelected+="'"+srvSel+"',";
			}
		serviceSelected = serviceSelected.substring(0,serviceSelected.lastIndexOf(","));
		sb.append("service_selected IN( "+serviceSelected+")");
		
		String catCustomQry = sb.toString();
		...
		String sql = "SELECT * from user_documents where "+catCustomQry+" order by user_document_id desc limit 10";
		System.out.println("Content sql : "+sql);
		return jdbcTemplate.query(sql, new RowMapper<UserDocument>() {
			@Override
			public UserDocument mapRow(ResultSet rs, int rownumber) throws SQLException {
	...
	public List<UserDocument> findDocumentsByServiceSelected(String serviceSelected) {
		
		StringBuffer sb = new StringBuffer();
		
		String serviceSelectedQry = "others";
		if (serviceSelected != null && serviceSelected.length() > 0) {
			serviceSelectedQry = "service_selected= '"+serviceSelected+"'";
		}
		
		String sql = "SELECT * from user_documents where "+serviceSelectedQry+" order by user_document_id desc";
		System.out.println("Content sql : "+sql);
		return jdbcTemplate.query(sql, new RowMapper<UserDocument>() {
			@Override
			public UserDocument mapRow(ResultSet rs, int rownumber) throws SQLException {
...
public List<UserDocument> findDocumentsByServiceSelectedAndTitle(String serviceSelected,String title) {
		...
		String sql = "SELECT * from user_documents where "+serviceSelectedQry+titleQry+" order by user_document_id desc";
		System.out.println("Content sql : "+sql);
		return jdbcTemplate.query(sql, new RowMapper<UserDocument>() {
			@Override
			public UserDocument mapRow(ResultSet rs, int rownumber) throws SQLException {
